...
class Orders1 extends Admin_Controller 
{
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Orders1';

		$this->load->model('model_orders1');
	...
	*/
	public function index()
	{
		...
		$this->data['page_title'] = 'Manage Orders1';
		$this->render_template('orders1/index1', $this->data);		
	}
	...
	public function fetchOrdersData()
{
    ...
    $data = $this->model_orders1->getOrdersData();
    ...
    foreach ($data as $key => $value) {
        ...
        if(in_array('viewOrder', $this->permission)) {
            $buttons .= '<a target="__blank" href="'.base_url('orders1/printDiv/'.$value['id1']).'" class="btn btn-default"><i class="fa fa-print"></i></a>';
        }
        ...
        if(in_array('deleteOrder', $this->permission)) {
            $buttons .= ' <button type="button" class="btn btn-default" onclick="removeFunc('.$value['id1'].')" data-toggle="modal" data-target="#removeModal"><i class="fa fa-trash"></i></button>';
        }

        // gambar
        $gambar = '-';
        if(!empty($value['gambar1'])) {
            $gambar = '<img src="'.base_url('uploads/'.$value['gambar1']).'" width="60">';
        }

        // data table (JUMLAH HARUS SAMA DENGAN THEAD)
        $result['data'][$key] = array(
            $no,
            $value['customer_name1'],
	...
	*/
	public function create1()
	{
		...
		$this->data['page_title'] = 'Add Order1';
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	$order_id = $this->model_orders1->create1();
        	...
        	if($order_id) {
        		...
        		redirect('orders1/create1/', 'refresh');
        	}
        }
        else {
            // false case
            ...
            $this->render_template('orders1/create1', $this->data);
        }	
	}
...
public function update1($id = null)
{
    ...
    if($this->form_validation->run() == TRUE){
        ...
        $this->render_template('orders1/edit1',$data);
    }
}
	/*
	...
	*/
	public function remove()
	{
        ...
        $response = array();
        if($order_id) {
            $delete = $this->model_orders1->remove($order_id);
            if($delete == true) {
	...
	*/
	public function printDiv($id1)
	{
		...
		if($id1) {
			$order_data = $this->model_orders1->getOrdersData($id1);
			$orders_items = $this->model_orders1->getOrdersItemData($id1);
